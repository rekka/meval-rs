...
use nom::{IResult, Needed, alpha, multispace};
...
named!(var<Token>, map!(map_res!(alpha, std::str::from_utf8), |s: &str| Token::Var(s.into())));
    ...
    while !s.is_empty() {
        ...
        match r {
            ...
            Error(Err::Position(_, p)) => {
                let (i, _) = nom::slice_to_offsets(input, p);
